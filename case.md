...
2. Proximal control / clamp.
3. Expose and clear the field (irrigation, retractors, dabbing).
4. Prepare the ends.
5. Anastomosis (7-0).
6. Unclamp; restore flow.
7. Duplex check.
8. If pursed → reclamp, revise, unclamp, duplex again.
9. Close (subcutaneous + skin), dress.
...
3. **Proximal clamp on before “field went quiet” / reflection / arterial-line talk** (Act 2). The click, dry field, and Mark’s “you saw the spike?” only work after the clamp. Water-hammer / bruit / “we don’t instrument for sport” sits on that shut-in. Slide irrigation and exposure around the talk; do not reverse clamp and reflection.

4. **Anastomosis underway (or clearly starting) before inverse-problem / “sewing without watching flow” talk** (Act 5). Sarah announces the join, warns Stuart about pursing, tells Mark the duplex comes after. Mark’s history-matching speech needs that condition: join now, flow later. Do not duplex-first then invent the sew.

5. **Anastomosis complete → unclamp → first duplex before “pursed / revise”** (Act 5). Bad waveform (stenotic rasp, peak velocity up) forces clamp back on and revision. Good triphasic trace only after the revise and second unclamp/duplex. Do not praise a clean waveform before the pursing beat has played, and do not revise without a failed check that motivates it.
...
## Where each act sits (current script)
...
(Filenames lag the on-page act numbers for Acts 4–5; follow the script titles when ordering the evening.)
...
## Sideways paths (in play or available)

Already used:

- Accident storytelling while the block sets.
- Sterile marker + drape-backing sketch (well / annulus).
- Mark’s clipboard notepad under the lights (narrowing, network, pulsatile curves, pipe vs vessel).
- Oil↔medicine maps that land (shut-in/clamp, choke/stenosis, ECD/friction, mud ↔ shear-thinning blood, duplex ↔ wellhead gauges).
- Maps that **fail honestly** (cement plug ≠ intima; cascade still rhymes).
- Counterfactual monitoring (what an arterial line *would* show) shut down by Elena’s lane.
- Cavitation → mechanical valves / hemolysis.
- Failed anastomosis → revise → choke recognition from Mark.

Available if pacing needs a turn without new plot:

- Another irrigation / exposure beat instead of new dialogue.
